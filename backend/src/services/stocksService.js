...
exports.addStockEntry = async ({ part_id, qty, type, ref, user_id }) => {
    await sequelize.transaction(async (t) => {
        const st = await Stock.findOne({ where: { part_id }, transaction: t, lock: t.LOCK.UPDATE });
        ...
        await StockEntry.create({ part_id, qty, type, ref, created_by: user_id }, { transaction: t });
    });
};
